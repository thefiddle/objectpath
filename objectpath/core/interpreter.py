...
import objectpath_pkg.objectpath.utils.colorify as color  # pylint: disable=W0614
        ...
        def exe(node):
            ...
            # 		return fst <= snd and snd or False
                ...
                if typefst in STR_TYPES:
                    ...
                    if D: self.info("doing pure python comparison '%s' is '%s'", fst, snd)
                    ret = fst == snd
                # else:
                # 	try:
                # 		global ObjectId
            ...
            # 	return self.node
                ...
                if len_node is 3:  # selector used []
                    ...
                    if selectorIsTuple and selector[0] in SELECTOR_OPS:
                        if D: self.debug("found %s operator in selector, %s", color.bold(selector[0]),
                                         color.bold(selector))
                        if type(fst) is dict:
                            fst = [fst]
                ...
                # string
                    ...
                    if not escape:
                        from objectpath_pkg.objectpath.utils import escape, escapeDict
                    return escape(args[0], escapeDict)
                elif fnName == "unescape":
                    global unescape, unescapeDict
                    if not unescape:
                        from objectpath_pkg.objectpath.utils import unescape, unescapeDict
